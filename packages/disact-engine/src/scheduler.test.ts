...
describe("createScheduler", () => {
  ...
  describe("requestRerender", () => {
    it("タスク実行中にrequestRerenderを呼び出すとタスク完了後に再実行される", async () => {
      let taskCount = 0;
    ...
    it("タスク実行中に複数回requestRerenderを呼び出しても1回の再実行にまとめられる", async () => {
      let taskCount = 0;
      let scheduler: ReturnType<typeof createScheduler>;
    ...
    it("アイドルタイムアウト中にrequestRerenderを呼び出すとアイドルタイムアウトが中断され再実行される", async () => {
      let taskCount = 0;
      let scheduler: ReturnType<typeof createScheduler>;
      const onIdle = vi.fn();
      ...
      // アイドルタイムアウト中に requestRerender を呼び出す（タイムアウト発火前）
      await vi.advanceTimersByTimeAsync(50);
      scheduler.requestRerender();

      // アイドルタイムアウトが中断され、新しいタスクが実行される
      await vi.advanceTimersByTimeAsync(10);
      expect(taskCount).toBe(2);

      // onIdle はまだ呼ばれていない（タイムアウトが中断されたため）
      expect(onIdle).not.toHaveBeenCalled();

      // 2回目のタスク後のアイドルタイムアウトを経過させる
      await vi.advanceTimersByTimeAsync(110);
      expect(onIdle).toHaveBeenCalledTimes(1);
    });

    it("disposeされた後にrequestRerenderを呼び出してもエラーにならない", async () => {
      const task = vi.fn();
      const scheduler = createScheduler({ task });

      scheduler.dispose();

      expect(() => scheduler.requestRerender()).not.toThrow();
      expect(task).not.toHaveBeenCalled();
    });
  });
